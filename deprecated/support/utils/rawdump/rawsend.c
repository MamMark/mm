/*
 * rawSend - send directly on serial connection.  (testing)
 *
...
#define VERSION "rawsend: v0.0.1  (13 Sep 2010)\n"
...
static void usage(char *name) {
  fprintf(stderr, "usage: %s [-Dv]           <serial device>  <baud rate>\n", name);
  fprintf(stderr, "       %s [-Dv] --serial  <serial device>  <baud rate>\n", name);
  ...
  fprintf(stderr, "  -r           raw, send direct to serial port (default)\n");
  exit(2);
}
