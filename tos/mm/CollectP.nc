...
implementation {
  ...
  /*
   * Always write the reboot sequence first.
   *
   * SYNC/R
   * REBOOT
   * VERSION
   *
   * This sequence gets written to the first sector found by
  ...
  command bool DblkResyncOffset.set_value(uint32_t *t, uint32_t *l) {
    ...
    if (!l || !t || *l != sizeof(uint32_t))
      call Panic.panic(0, 0, 0, 0, 0, 0);
    err = call Resync.start(t, *t + (2 * SYNC_MAX_SECTORS * SD_BLOCKSIZE));
