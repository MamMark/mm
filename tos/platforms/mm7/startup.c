...
uint32_t start_jiffies, start_ps;
 ...
 *
 * The dev6a doesn't have power switches and regulators that are majorly
...
void __map_ports() {
  ...
  P2MAP->PMAP_REG[1] = PMAP_UCA0CLK;
  P2MAP->PMAP_REG[2] = PMAP_UCA0SOMI;
  P2MAP->PMAP_REG[3] = PMAP_UCA0SIMO;
  P3MAP->PMAP_REG[0] = PMAP_UCA2SOMI;
  P3MAP->PMAP_REG[1] = PMAP_UCA2CLK;
  P3MAP->PMAP_REG[2] = PMAP_UCA2SIMO;
  P3MAP->PMAP_REG[4] = PMAP_UCB2SIMO;
  P3MAP->PMAP_REG[5] = PMAP_UCB2SOMI;
 ...
 */
void __pins_init() {
  PA->OUT  = 0;                 /* zero the registers we touch */
  PB->OUT  = 0;                 /* P4/P3  */
  PC->OUT  = 0;                 /* P6/P5  */
  PD->OUT  = 0;                 /* P8/P7  */
  PE->OUT  = 0;                 /* P10/P9 */
  ...
  /* DockComm is on P2.{1,2,3}, pull downs */
  P2->DIR  = 0xF0;
  P2->REN  = 0x0F;

  /* sd0/radio spi, radio_csn */
  P3->OUT  = 0x08;
  P3->DIR  = 0x08;
  P3->SEL0 = 0x77;

  /* 4.2 is ACLK 32KHz for gps RTCI */
  P4->OUT  = 0x80;
  P4->DIR  = 0x8F;
  P4->SEL0 = 0x04;
  ...
  P6->DIR  = 0x3F;
  P6->SEL1 = 0xC0;              /* connect to UCB3 I2C            */

  P7->DIR  = 0xF0;
  P7->SEL0 = 0x0D;              /* gps SPI                        */

  P8->DIR  = 0xFD;              /* drive pwr_sd0_en               */

  P9->DIR  = 0xff;              /* all out, low                   */
  P10->DIR = 0xff;              /* all out, low                   */

  /* pwr gps, and gps_vbckup */
  PJ->OUT  = 0x14;
  PJ->DIR  = 0x16;              /* LFXOUT output */

  /*
   * on reset, PJ.4 (gps_pwr) is set to be TDI.  We want it to be gps_pwr and
   * a digital port.  Turn off the SEL0 bit for PJ.4 to switch back to DIO.
   */
  BITBAND_PERI(UBX_PWR_PORT->SEL0, UBX_PWR_PIN) = 0;
}
...
void __system_init(bool disable_dcor) {
  ...
  __start_time();
  start_jiffies = __platform_jiffiesRaw();
  start_ps      = __coretime_get_ps();
  nop();
}
...
void __Reset() {
  ...
  if (ow_control_block.rtc_src < RTCSRC_GPS0)
