...
#warning CHECK SPI clock rate presented to gps chip
#endif
...
#define  gps_warn(where, arg)      do { \
  ...
  /*
   * We use SPI to communicate with the ublox gps chip.  It is clocked by
   * the EUSCI which is clocked by SMCLK (DCO/2).   With a DCO of 16 MiHz
   * and a MSP432_UBLOX_DIV of 1, we will see 8 MiHz at SCLK.  It should
   * not exceed 10 MHz.
   */
  ...
  /* reset pin not connected on the MM7 */
  command void HW.gps_set_reset() { }
  command void HW.gps_clr_reset() { }
