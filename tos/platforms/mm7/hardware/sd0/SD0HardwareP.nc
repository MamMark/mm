...
ctlw0 : (  EUSCI_A_CTLW0_CKPL        | EUSCI_A_CTLW0_MSB  |
  ...
  async command error_t AsyncInit.init() {
    ...
    /* make sure pins are in a resonable state */
    call HW.sd_off();
...
#define SG_SIZE 32
  ...
  /*
   * When the SD is turned off we do not want to power the chip via any of
   * its inputs being driven high.  When we power off, set any pins that are
   * connected to the SD to inputs.  We switch the pins from the EUSCI to
   * a simple port so we can control what the pins are doing.
   *
   * We don't need to worry about pulling the signals to a particular state
   * because the SD has pull ups that act as clamps when the SD is powered
   * down.
   */
  async command void HW.sd_on() {
    SD0_CSN   = 1;                      /* make sure tristated */
    BITBAND_PERI(SD0_CSN_PORT->DIR, SD0_CSN_PIN) = 1; /* csn output */
    SD0_PWR   = 1;
    P3->SEL0 |= 0x07;                   /* somi, sclk, simo to Module */
  }
  ...
  async command void HW.sd_off() {
    ...
    BITBAND_PERI(SD0_CSN_PORT->DIR, SD0_CSN_PIN) = 0; /* csn input */
    P3->SEL0 &= 0xF8;                   /* low 3 bits 0 */
    SD0_PWR   = 0;
  }

  async command bool HW.isSDPowered() { return SD0_PWRD_P; }
