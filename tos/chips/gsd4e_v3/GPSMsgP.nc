...
implementation {
  ...
  norace collect_state_t collect_state;		// message collection state
  norace uint16_t collect_length;		// length of payload
  norace uint16_t        collect_cur_chksum;	// running chksum of payload

  norace uint8_t  collect_msg[GPS_BUF_SIZE];
  norace uint8_t  collect_overflow[GPS_OVR_SIZE];
  norace uint8_t  collect_nxt;		        // where we are in the buffer
  norace uint8_t  collect_left;			// working copy
  norace bool     on_overflow;
  norace bool     got_fix;
  norace uint32_t gps_on_time;
   ...
   * Error counters
   */
  norace uint16_t collect_overflow_full;
  norace uint8_t  collect_overflow_max;
  norace uint16_t collect_too_big;
  norace uint16_t collect_chksum_fail;
  norace uint16_t collect_proto_fail;
  norace uint32_t last_surfaced;
  norace uint32_t last_submerged;
