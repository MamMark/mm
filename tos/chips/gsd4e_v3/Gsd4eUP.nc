...
#ifdef GPS_RO
noinit uint8_t gps_speed;		// will default to 0, 57600 (1 is 9600, 2 is 115200 (if compiled in))
noinit uint8_t ro;
#endif
...
typedef enum {
   ...
   * 57600-SirfBin.  (Operational Speed).
   *
  ...
  GPSC_RECONFIG_9600_PWR_DOWN,		// power down
  GPSC_RECONFIG_9600_START_DELAY,	// gps power on
  GPSC_RECONFIG_9600_HUNTING,		// looking for '$', nmea start
  GPSC_RECONFIG_9600_EOS_WAIT,		// waiting for end of start
  GPSC_RECONFIG_9600_SENDING,		// waiting for send of go_sirf_bin to complete
...
//    call LogEvent.logEvent(DT_EVENT_GPS_OFF, 0);
  ...
  task void gps_config_task() {
	...
	return;
      ...
      case GPSC_RECONFIG_9600_EOS_WAIT:
...
//	signal GPSBoot.booted();
   ...
   * and the GPS will revert to NMEA-9600-8N1.  This will also be the case the
   ...
   * to 9600 and switch over using the NMEA change protocol message.
   *
   * at 9600 NMEA, first char must be '$' (0x24, 36)
   ...
   *
   * gps @ 9600, 9600: TTFC:  start up message length: 250 chars, 724 mis, 330 from gps power on
...
//    call LogEvent.logEvent(DT_EVENT_GPS_START, 0);
  ...
  event void GPSTimer.fired() {
    ...
    switch (gpsc_state) {
      ...
      case GPSC_FAIL:
      case GPSC_OFF:
      case GPSC_RECONFIG_9600_HUNTING:		// timed out.  no start char
      case GPSC_RECONFIG_9600_SENDING:		// timed out send.
	...
	if ((err = call HW.send_block(sirf_send_boot, sizeof(sirf_send_boot))))
	...
	return;
      ...
      case GPSC_RECONFIG_9600_PWR_DOWN:
	/*
	 * Doing a reconfig (see start_reconfig for sequence).
	 * gps was powered down.  Bring it back up and look for the
	 * start up sequence.  Should be NMEA@9600
	 */
	gpsc_change_state(GPSC_RECONFIG_9600_START_DELAY, GPSW_TIMER);
	call HW.gps_pwr_on();
	t_gps_pwr_on = call LocalTime.get();
	call GPSTimer.startOneShotAt(t_gps_pwr_on, DT_GPS_PWR_UP_DELAY);
        call HW.gps_speed_di(9600);
	...
	gpsc_change_state(GPSC_RECONFIG_9600_HUNTING, GPSW_TIMER);
	call GPSTimer.startOneShot(DT_GPS_HUNT_LIMIT);
        call HW.gps_rx_int_enable();
	return;

      case GPSC_RECONFIG_9600_EOS_WAIT:
	gpsc_change_state(GPSC_RECONFIG_9600_SENDING, GPSW_TIMER);
	call GPSTimer.startOneShot(DT_GPS_SEND_TIME_OUT);
	if ((err = call HW.send_block(nmea_go_sirf_bin, sizeof(nmea_go_sirf_bin))))
	 ...
	 */
	call HW.send_block(sirf_send_start, sizeof(sirf_send_start));
	return;
	 ...
	 * . turn off interrupts
	 * . turn off gps power
	 * . force gpsc_state to 9600_PWR_DOWN
	 * . kick gps_config_task (gpsc_state tells it what to do).
	 */
        call HW.gps_rx_int_disable();	// going down, no interrupts while there
	 ...
	 *
	 * Once we get here, ints are off and we force to RECONFIG_9600_PWR_DOWN.  Once
	...
	if (gpsc_reconfig_trys) {
	  gpsc_reconfig_trys--;
	  gpsc_change_state(GPSC_RECONFIG_9600_PWR_DOWN, GPSW_TIMER);
...
//	  call LogEvent.logEvent(DT_EVENT_GPS_RECONFIG, gpsc_reconfig_trys);
  ...
  void process_byte(uint8_t byte) {
	...
	return;
       ...
       */
      case GPSC_RECONFIG_9600_EOS_WAIT:
      case GPSC_RECONFIG_9600_SENDING:
      case GPSC_EOS_WAIT:
      case GPSC_SENDING:
      case GPSC_FINI_WAIT:
      case GPSC_FINISH:
	...
	return;
      ...
      case GPSC_RECONFIG_9600_HUNTING:
	if (byte == NMEA_START) {
	  /*
	   * we aren't very robust here.  we just assume if we see the
	   * $ then all is good.
	   */
	  gpsc_change_state(GPSC_RECONFIG_9600_EOS_WAIT, GPSW_RXBYTE);
	  post gps_config_task();
	  return;
	}
	return;
