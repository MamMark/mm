...
1. Edit the 'TagName Mappings' file on Google Drive.  ('Tag Stuff/Design Documents/TagName Mappings')
  - Typically copy and edit an existing line
2. Download the spreadsheet as a TSV file, File->Download->tab seperated values (tsv).  Name will be something
   like 'TagName Mappings - Wip.tsv'.   Rename to 'tos/comm/TagNames/TagNames.tsv`
3. Change directory to `mm/tos/comm/TagNames`
4. Invoke the preprocessor with the following command (factspp can be found in tools/tagnet/factspp)
```
 factspp -o . TagNames.tsv
```
Results will be found in `mm/tos/comm/TagNames`.  This determines what objects are provided by the TagFS network stack.
...
## Wiring new nodes into the application.

For this example, let's assume that we are creating two new nodes, ```DblkBootRecNum``` and ```DblkBootOffset```,
both provided by CollectP on the Tag.
...
```
CollectP/CollectC have been edited to provide the code implementing DblkBootRecNum/Offset.

```factspp``` is used to generate Tagnet linkages from TagNames.tsv it will create the following in TagnetC.nc.
TagnetC is generated by factspp and the adapter numbers can change.  TagNames.tsv has been editted to define
various attributes for the DblkBootRecNum/Offset objects.

One of the outputs of factspp is TagnetC which provides the wiring for the Tagnet network stack.  In our example,
...
The final missing piece connects the network stack to the provider of the defined objects.  The top level
